...
the same day, and again on 2026-09-04 and 2026-09-18 — `./scripts/verify-site.sh`
PASS on each, 780 no-JS words, all assets reachable. `./scripts/check-domain.sh`
on 2026-09-18 still reports `CNAME -> external.notion.site` and Notion serving,
which is the expected pre-cutover state. Nothing has regressed in two weeks.
